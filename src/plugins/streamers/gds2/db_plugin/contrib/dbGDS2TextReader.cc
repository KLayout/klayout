...
void GDS2ReaderText::init (const db::LoadLayoutOptions &options)
{
  GDS2ReaderBase::init (options);
  storedRecId = 0;
}

void GDS2ReaderText::unget_record (short rec_id)
{
  storedRecId = rec_id;
}

short GDS2ReaderText::get_record ()
{
...
void GDS2ReaderText::vConvertToXY (const std::string &_sArg)
{
...
short GDS2ReaderText::siExtractData (std::string &_sInput, std::string &_sToken, std::string &_sArguments)
{
  short token = 0;
...
short GDS2ReaderText::get_short ()
{
  int x = 0;
  if (! reader.try_read (x)) {
    error (tl::to_string (tr ("Expected an integer number")));
...
void GDS2ReaderText::error (const std::string &msg)
{
  throw GDS2ReaderTextException (msg, int (sStream.line_number ()), cellname ().c_str (), sStream.source ());
}

void GDS2ReaderText::warn (const std::string &msg, int wl)
{
  if (warn_level () < wl) {
    return;
  }
...
void GDS2ReaderText::get_time (unsigned int *mod_time, unsigned int *access_time)
{
...
void GDS2ReaderText::progress_checkpoint ()
{
