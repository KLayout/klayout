...
void DXFWriter::write (const db::Layout &layout, const db::Cell &cref, const std::set<db::cell_index_type> &cell_set, const std::vector<std::pair<unsigned int, db::LayerProperties>> &layers, double sf)
{
  //  instances
    ...
    if (cell_set.find (inst->cell_index ()) != cell_set.end ()) {
      ...
      for (db::Cell::cell_inst_array_type::iterator pp = inst->begin (); ! pp.at_end (); ++pp) {
        ...
        *this << 0 << endl
              << "INSERT" << endl;
        *this << 8 << endl
              << 0 << endl; // required by TrueView
        *this << 2 << endl
              << layout.cell_name (inst->cell_index ()) << endl;
        *this << 10 << endl
              << d.x () * sf << endl;
        *this << 20 << endl
              << d.y () * sf << endl;
...
void DXFWriter::write (db::Layout &layout, tl::OutputStream &stream, const db::SaveLayoutOptions &options)
{
  ...
  *this << 70 << endl
  ...
  for (std::vector<std::pair<unsigned int, db::LayerProperties>>::const_iterator l = layers.begin (); l != layers.end (); ++l) {
    ...
    *this << 0 << endl
          << "LAYER" << endl;
    *this << 70 << endl
          << 0 << endl; //  flags: seems to be required by some tools
    *this << 62 << endl
          << color << endl; //  color
    *this << 6 << endl
          << linestyle << endl; //  line style
    *this << 2 << endl;
    emit_layer (l->second);
...
void DXFWriter::emit_layer (const db::LayerProperties &lp)
{
...
void DXFWriter::write_texts (const db::Layout & /*layout*/, const db::Cell &cell, unsigned int layer, double sf)
{
  ...
  while (! shape.at_end ()) {
    ...
    if (use_mtext) {
      ...
      *this << 50 << endl
            << (shape->text_trans ().rot () % 4) * 90.0 << endl;
...
void DXFWriter::write_polygons (const db::Layout & /*layout*/, const db::Cell &cell, unsigned int layer, double sf)
{
...
void DXFWriter::write_polygon (const db::Polygon &polygon, double sf)
{
  if (polygon.holes () > 0 && (m_options.polygon_mode == 0 || m_options.polygon_mode == 1 || m_options.polygon_mode == 2)) {
    ...
    *this << 91 << endl
          << polygon.holes () + 1 << endl;

    for (unsigned int c = 0; c < polygon.holes () + 1; ++c) {
      *this << 92 << endl
            << 3 << endl; // external polyline
      *this << 72 << endl
            << 0 << endl; // has bulge flag
      *this << 73 << endl
            << 1 << endl; // closed flag
      *this << 93 << endl
            << polygon.contour (c).size () << endl;
...
void DXFWriter::write_boxes (const db::Layout & /*layout*/, const db::Cell &cell, unsigned int layer, double sf)
{
...
void DXFWriter::write_paths (const db::Layout & /*layout*/, const db::Cell &cell, unsigned int layer, double sf)
{
