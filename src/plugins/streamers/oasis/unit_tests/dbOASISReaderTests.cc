...
void compare_ref (tl::TestBase *_this, const char *test, const db::Layout &layout)
{
...
void run_test (tl::TestBase *_this, const char *test)
{
  db::Manager m (false);
  db::Layout layout (&m);
...
void run_test_error (tl::TestBase *_this, const char *test, const char *msg_au)
{
  db::Manager m (false);
  db::Layout layout (&m);
  std::string fn (tl::testdata ());
