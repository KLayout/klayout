...
void run_test_bool (tl::TestBase *_this, const char *file, int mode, bool min_coherence = true, const char *au_file1 = 0, const char *au_file2 = 0, const char *au_file3 = 0)
{
  ...
  for (unsigned int l = 0; l < layout_org.layers (); ++l) {
    if (layout_org.is_valid_layer (l) && layout_org.get_properties (l).layer == 1 && layout_org.get_properties (l).datatype == 0) {
      la = l;
      break;
    }
  }
  ...
  int lb = -1;
  for (unsigned int l = 0; l < layout_org.layers (); ++l) {
    if (layout_org.is_valid_layer (l) && layout_org.get_properties (l).layer == 2 && layout_org.get_properties (l).datatype == 0) {
      lb = l;
      break;
    }
  }
  ...
  int lr = -1;
  for (unsigned int l = 0; l < layout_org.layers (); ++l) {
    if (layout_org.is_valid_layer (l) && layout_org.get_properties (l).layer == 100 && layout_org.get_properties (l).datatype == 0) {
      lr = l;
      break;
    }
  }
...
void run_test_twobool (tl::TestBase *_this, const char *file, const char *au_file)
{
  ...
  int lrb = -1;
  for (unsigned int l = 0; l < layout_org.layers (); ++l) {
    if (layout_org.is_valid_layer (l) && layout_org.get_properties (l).layer == 101 && layout_org.get_properties (l).datatype == 0) {
      lrb = l;
      break;
    }
  }
