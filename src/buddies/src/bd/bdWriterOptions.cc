...
void GenericWriterOptions::init_from_options (const db::SaveLayoutOptions &save_options_nc)
{
...
void GenericWriterOptions::add_options (tl::CommandLineOptions &cmd, const std::string &format)
{
  std::string group ("[Output options - General]");

  cmd << tl::arg (group +
                    "-os|--scale-factor=factor",
                  &m_scale_factor, "Scales the layout upon writing",
  ...
  if (format.empty ()) {
    cmd << tl::arg (group +
                      "-of|--format=format",
                    &m_format, "Specifies the output format",
  ...
  if (format.empty () || format == gds2_format_name || format == gds2text_format_name || format == oasis_format_name) {
    cmd << tl::arg (group +
                      "-od|--dbu-out=dbu",
                    &m_dbu, "Uses the specified database unit",
    ...
    cmd << tl::arg (group +
                      "-ol|--libname=libname",
                    &m_libname, "Uses the given library name",
  ...
  cmd << tl::arg (group +
                    "#--drop-empty-cells",
                  &m_dont_write_empty_cells, "Drops empty cells",
  ...
  if (format.empty () || format == gds2_format_name || format == gds2text_format_name) {
    cmd << tl::arg (group +
                      "#--keep-instances",
                    &m_keep_instances, "Keeps instances of dropped cells",
  ...
  if (format.empty () || format == gds2_format_name || format == gds2text_format_name || format == oasis_format_name) {
    cmd << tl::arg (group +
                      "!#--no-context-info",
                    &m_write_context_info, "Does not write context information",
  ...
  cmd << tl::arg (group +
                    "#--write-cells=sel",
                  &m_cell_selection, "Specifies cells to write",
  ...
  if (format.empty () || format == gds2_format_name || format == gds2text_format_name) {
    ...
    cmd << tl::arg (group +
                      "-ov|--max-vertex-count=count",
                    &m_gds2_max_vertex_count, "Specifies the maximum number of points per polygon",
        ...
        << tl::arg (group +
                      "#--multi-xy-records",
                    &m_gds2_multi_xy_records, "Allows unlimited number of points",
        ...
        << tl::arg (group +
                      "-ow|--resolve-skew-arrays",
                    &m_gds2_resolve_skew_arrays, "Resolve skew (non-orthogonal) arrays",
        ...
        << tl::arg (group +
                    ...
                    &m_gds2_no_zero_length_paths, "Converts zero-length paths to polygons",
        ...
        << tl::arg (group +
                      "-on|--cellname-length=length",
                    &m_gds2_max_cellname_length, "Limits cell names to the given length",
                    "If this option is given, long cell names will truncated if their length exceeds the given length.")
        << tl::arg (group +
                      "#--user-units=unit",
                    &m_gds2_user_units, "Specifies the user unit to use",
                    "Specifies the GDS2 user unit. By default micrometers are used for the user unit.")
        << tl::arg (group +
                      "#!--no-timestamps",
                    &m_gds2_write_timestamps, "Don't write timestamps",
        ...
        << tl::arg (group +
                    ...
                    &m_gds2_write_cell_properties, "Write cell properties",
        ...
        << tl::arg (group +
                    ...
                    &m_gds2_write_file_properties, "Write file properties",
                    "This option enables a GDS2 extension that allows writing of file properties to GDS2 files. "
                    "Consumers that don't support this feature, may not be able to read such a GDS2 files.")
        << tl::arg (group +
                      "#--default-text-size",
                    &m_gds2_default_text_size, "Default text size",
  ...
  if (format.empty () || format == oasis_format_name) {
    ...
    cmd << tl::arg (group +
                      "-ok|--compression-level=level",
                    &m_oasis_compression_level, "Specifies the OASIS compression level",
        ...
        << tl::arg (group +
                      "-ob|--cblocks",
                    &m_oasis_write_cblocks, "Uses CBLOCK compression",
        ...
        << tl::arg (group +
                      "-ot|--strict-mode",
                    &m_oasis_strict_mode, "Uses strict mode",
        ...
        << tl::arg (group +
                      "#--recompress",
                    &m_oasis_recompress, "Compresses shape arrays again",
        ...
        << tl::arg (group +
                      "#--permissive",
                    &m_oasis_permissive, "Permissive mode",
        ...
        << tl::arg (group +
                      "#--write-std-properties",
                    &m_oasis_write_std_properties, "Writes some global standard properties",
        ...
        << tl::arg (group +
                      "#--subst-char=char",
                    this, &GenericWriterOptions::set_oasis_substitution_char, "Specifies the substitution character for non-standard characters",
  ...
  if (format.empty () || format == lstream_format_name) {
    ...
    cmd << tl::arg (group +
                      "-oc|--lstr-compression-level=level",
                    &m_lstream_compression_level, "Specifies the LStream compression level",
        ...
        << tl::arg (group +
                      "#--lstr-recompress",
                    &m_lstream_recompress, "Compresses shape arrays again",
                    "With this option, shape arrays will be expanded and recompressed. This may result in a better "
                    "compression ratio, but at the cost of slower execution.")
        << tl::arg (group +
                      "#--lstr-permissive",
                    &m_lstream_permissive, "Permissive mode",
                    "In permissive mode, certain forbidden objects are reported as warnings, not as errors: "
                    "paths with odd width, polygons with less than three points etc.");
  }
  ...
  if (format.empty () || format == dxf_format_name) {
    ...
    cmd << tl::arg (group +
                      "-op|--polygon-mode=mode",
                    &m_dxf_polygon_mode, "Specifies how to write polygons",
  ...
  if (format.empty () || format == mag_format_name) {
    ...
    cmd << tl::arg (group +
                      "--magic-lambda-out=lambda",
                    &m_magic_lambda, "Specifies the lambda value when writing Magic files (which are unitless)")
        << tl::arg (group +
                      "--magic-tech",
                    &m_magic_tech, "Specifies the technology to include in the Magic files");
  }
}
...
void GenericWriterOptions::configure (db::SaveLayoutOptions &save_options, const db::Layout &layout) const
{
